...
pub fn parse_sql_(sql: String) -> Option<(Vec<String>, String)> {
    // Regex to capture columns and table name
    let re =
        Regex::new(r"(?i)select\s+(?P<columns>[\w\s,\(\)\*]+)\s+from\s+(?P<table>\w+)").unwrap();
    ...
    if let Some(caps) = re.captures(&sql) {
        ...
        Some((columns, String::from(table)))
    } else {
        None
    }
}
...
mod tests {
    ...
    fn test_parser() {
        ...
        let (columns_apples, table_name_apples) =
        ...
        let (columns_oranges, table_name_oranges) =
    ...
    fn test_parser_sql_superhoreos() {
        ...
        // Test parsing
        let (columns, table) = parse_sql_(sql_.clone()).expect("Failed to parse superheroes query");
    ...
    fn test_parser_sql_companies() {
        ...
        // Test parsing
        let (columns, table) = parse_sql_(sql_.clone()).expect("Failed to parse companies query");
        assert_eq!(columns, vec!["COUNT(*)".to_string()]);
        assert_eq!(table, db_name);
    }
    ...
    fn test_parser_sql_() {
        ...
        for (sql, expected_columns, expected_table) in test_cases {
            if let Some((columns, table)) = parse_sql_(sql.to_string()) {
